...
namespace MongoDB.Driver.Core.Authentication.Oidc
{
    ...
    internal class OidcProvidersCache : IOidcProvidersCache
    {
        ...
        public IOidcExternalAuthenticationCredentialsProvider GetProvider(OidcInputConfiguration inputConfiguration)
        {
            ...
            lock (_lock)
            {
                if (_providersCache.TryGetValue(inputConfiguration, out var value))
                {
                    value.Touch();
                }
                else
                {
                    value = new OidcCacheValue(new OidcExternalAuthenticationCredentialsProvider(inputConfiguration, _clock), _clock);
                    _providersCache.Add(inputConfiguration, value);
                }
