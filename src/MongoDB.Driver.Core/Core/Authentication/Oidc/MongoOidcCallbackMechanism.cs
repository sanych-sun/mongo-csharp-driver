...
namespace MongoDB.Driver.Core.Authentication.Oidc
{
    internal sealed class MongoOidcCallbackMechanism : OidcSaslMechanism
    {
        ...
        public override ISaslStep Initialize(IConnection connection, SaslConversation conversation, ConnectionDescription description)
        {
            ...
            {
                var oidcCredentials = _oidsCredentialsProvider.CreateCredentialsFromExternalSource(stepResult.InvalidCredentials, stepResult.IdpServerInfo);
        ...
        // nested types
        private sealed class CallbackWorkflowClientFirst : ISaslStep
        {
            ...
            public ISaslStep Transition(SaslConversation conversation, byte[] bytesReceivedFromServer)
            {
                ...
                var oidcCredentials = _oidsCredentialsProvider.CreateCredentialsFromExternalSource(invalidCredentials: null, serverFirstMessageDocument);
                return _oidcSaslStepFactory.CreateLastSaslStep(oidcCredentials);
            }

            public async Task<ISaslStep> TransitionAsync(SaslConversation conversation, byte[] bytesReceivedFromServer, CancellationToken cancellationToken)
            {
                var serverFirstMessageDocument = BsonSerializer.Deserialize<BsonDocument>(bytesReceivedFromServer);
