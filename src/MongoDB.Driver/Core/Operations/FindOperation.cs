    ...
    internal sealed class FindOperation<TDocument> : IReadOperation<IAsyncCursor<TDocument>>, IExecutableInRetryableReadContext<IAsyncCursor<TDocument>>
    {
        ...
        public BsonDocument CreateCommand(OperationContext operationContext, ICoreSession session, ConnectionDescription connectionDescription)
        {
            ...
            return new BsonDocument
            {
                ...
                { "skip", _skip, v => v.Value, _skip.HasValue },
                { "limit", _limit, v => Math.Abs(v.Value), _limit.HasValue && _limit != 0 },
                { "batchSize", batchSize, v => v.Value, batchSize.HasValue && batchSize > 0 },
                { "singleBatch", true, _limit < 0 || _singleBatch.HasValue },
                { "comment", _comment, _comment != null },
                { "maxTimeMS", _maxTime, v => MaxTimeHelper.ToMaxTimeMS(v.Value), _maxTime.HasValue && !operationContext.IsRootContextTimeoutConfigured() },
                { "max", _max, _max != null },
                { "min", _min, _min != null },
                { "returnKey", _returnKey, v => v.Value, _returnKey.HasValue },
                { "showRecordId", _showRecordId, v => v.Value, _showRecordId.HasValue },
                { "tailable", true, _cursorType == CursorType.Tailable || _cursorType == CursorType.TailableAwait },
                { "oplogReplay", _oplogReplay, v => v.Value, _oplogReplay.HasValue },
                { "noCursorTimeout", _noCursorTimeout, v => v.Value, _noCursorTimeout.HasValue },
                { "awaitData", true, _cursorType == CursorType.TailableAwait },
                { "allowDiskUse", _allowDiskUse, v => v.Value, _allowDiskUse.HasValue },
                { "allowPartialResults", _allowPartialResults, v => v.Value, _allowPartialResults.HasValue && isShardRouter },
                { "collation", _collation, v => v.ToBsonDocument(), _collation != null },
